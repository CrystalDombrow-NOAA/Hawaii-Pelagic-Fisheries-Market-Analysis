...
#Figure 9. Annual share of local bigeye tuna consumption in Hawai'i. 
...
#--------------------
#Figure 12: Annual and monthly average ex-vessel prices (per pound) for Hawai??i 
#bigeye tuna landings, 2008-2019.  
#--------------------
require(lubridate)
...
MonthlyPriceBET <- Monthdat %>%
  ...
  select(year, month, Bigeye.tuna) %>%
  group_by(month) %>%
  mutate(monthly.avg = mean(Bigeye.tuna))


#Create figure.
MonthlyPriceBET_plot <- ggplot(MonthlyPriceBET, 
                          aes(x = month, y = monthly.avg, group = 1)) +
  geom_line(stat = "identity") +
  geom_point() +
  scale_y_continuous(labels = scaleFUN) +
  labs(x = "Month", y = "Price") +
  coord_cartesian(ylim = c(0.00, 6.00)) +
  ggtitle("Monthly")+
  theme_classic() +
  theme(legend.position = "bottom", 
        legend.title = element_blank(),
...
#Create object for figure.
AnnualPriceBET <- Monthdat %>%
  mutate(date = start + months(0:143), year = year(date), 
         month = month(date, label = T, abbr = T)) %>%
  select(year, Bigeye.tuna) %>%
  group_by(year) %>%
  mutate(annual.avg = mean(Bigeye.tuna))

#Create figure.
AnnualPriceBET_plot <- ggplot(AnnualPriceBET, 
...
ExVessPrice_BET <- ggarrange(AnnualPriceBET_plot, MonthlyPriceBET_plot,
                             ncol = 2, nrow = 1)

#Save figure.
save_plot("Figures/ExVessPrice_BET.png", ExVessPrice_BET,
          base_aspect_ratio = 3)
...
#Save figure.
save_plot("Figures/BigeyeExports.png", Export_Plot, base_aspect_ratio = 2)
...
#Save figure.
save_plot("Figures/BigeyeImports.png", Import_Plot, base_aspect_ratio = 2)
